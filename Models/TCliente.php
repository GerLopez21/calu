...
trait TCliente{
	...
	public function insertDetalle(int $idpedido, int $productoid, float $precio, int $cantidad,string $talle=null,string $color =null){
		 ...
		 public function getStockAjax($talleseleccionado,$producto)
		{
		    ...
		    $this->strTalle = $talleseleccionado;
		    $this->intProducto = $producto;

			$sql = "SELECT * FROM talles where nombretalle like '$talleseleccionado'";

			$request = $this->con->select($sql);

            $idtalle = $request['idstocktalle'];
            ...
            where talleid = $idtalle and productoid = $this->intProducto and cantidad > 0";
