...
if(document.querySelector(".js-btn-variation")){
    ...
    optmetodo.forEach(function(optmetodo) {
      ...
      $('.js-insta-variation-label-material').html(''); 
        ...
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        let ajaxUrl = base_url+'/Tienda/getStockProductoAjax/'+talleseleccionado; 
        let formData = new FormData();
...
$(document).on('click','.js-btn-variation-color', function(){ //esta función se ejecutará en todos los casos
       ...
       $(".btn").css( "background-color", "white" );
        let seleccionadocolor = $(this).parent().find('.js-btn-variation-color').html();
        colorseleccionado = seleccionadocolor.trim();
...
if(document.querySelector("#formDatos")){
    ...
    formDatos.onsubmit = function(e) {
	    ...
	    console.log(strCalle)
	    console.log(intNumero)
	    console.log(strCiudad)
	    console.log(intCP)
	    console.log(strProvincia)
	    if(error == true){
